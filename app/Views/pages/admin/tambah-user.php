		...
		<div class="card-body">
			<div class="row">
				<div class="form-group col-md-6">
					...
					<input autocomplete="off" type="text" name="nama" value="<?= old('nama') ?>" class="form-control <?= session('errors.nama') ? 'is-invalid' : '' ?>" required>
					<?php if (session('errors.nama')): ?><div class="invalid-feedback text-danger"> <?= session('errors.nama') ?> </div><?php endif; ?>
				</div>

				<div class="form-group col-md-6">
					<label class="font-weight-bold">E-Mail</label>
					<input autocomplete="off" type="email" name="email" value="<?= old('email') ?>" class="form-control <?= session('errors.email') ? 'is-invalid' : '' ?>" required>
					<?php if (session('errors.email')): ?><div class="invalid-feedback text-danger"> <?= session('errors.email') ?> </div><?php endif; ?>
				</div>

				<div class="form-group col-md-6">
					<label class="font-weight-bold">Password</label>
					<input autocomplete="off" type="password" name="password" class="form-control <?= session('errors.password') ? 'is-invalid' : '' ?>" required>
				...
				<div class="form-group col-md-6">
					...
					<input autocomplete="off" type="password" name="konfirmasi" class="form-control <?= session('errors.konfirmasi') ? 'is-invalid' : '' ?>" required>
				...
				<div class="form-group col-md-6">
					...
					<?php if (session('errors.role')): ?><div class="invalid-feedback text-danger"> <?= session('errors.role') ?> </div><?php endif; ?>
				</div>
			</div>
		</div>
